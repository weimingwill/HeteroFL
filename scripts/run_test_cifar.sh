...
srun -u --partition=innova --job-name=${fraction}-cifar-heterofl \
    -n1 --gres=gpu:1 --ntasks-per-node=1 \
    python ${project_dir}/src/test_classifier_fed.py \
      --data_name CIFAR10 --model_name resnet18 \
      --control_name 1_100_${fraction}_non-iid-2_dynamic_a1-b1-c1-d1-e1_gn_0_0 2>&1 | tee log/users_test_cifar_${fraction}-${now}.log &
